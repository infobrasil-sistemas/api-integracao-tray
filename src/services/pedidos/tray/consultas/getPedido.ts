...
export async function getPedido(loja: ILojaTray, accessToken: string, id: number): Promise<IOrder | undefined> {
    try {
        const response = await axios.get(`${loja.LTR_API_HOST}/orders/${id}/complete`, {
            params: {
        ...
        throw new Error(`Erro ao obter dados do pedido ${id} da loja ${loja.LTR_CNPJ} -> ${error}`);
    }
}
