...
export async function enviarGrupo(loja: ILojaTray, conexao: any, accessToken: string, grupo: IGrupoNaoIntegrado) {
    try {
        ...
        if (axios.isAxiosError(error)) {
            logger.log({
                level: 'error',
                message: `Erro ao cadastrar grupo ${grupo.name} da loja ${loja.LTR_NOME} -> 
            ...
            logger.log({
                level: 'error',
                message: `Erro inesperado ao cadastrar grupo ${grupo.name} da loja ${loja.LTR_NOME} -> ${error.message}`
            });
        }
    }
