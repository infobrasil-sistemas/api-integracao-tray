...
export async function getCategoria(loja: ILojaTray, accessToken: string, id: number) {
    try {
        ...
        if (response.status === 201 || response.status === 200) {
            ...
            logger.log({
                level: 'error',
                message: `Erro ao buscar categoria ${id} da loja ${loja.LTR_NOME} -> ${response.data.message}`
            });
            throw new Error(`Erro ao buscar categoria ${id} da loja ${loja.LTR_NOME} -> ${response.data.message}`)
        }
    } catch (error) {
        logger.log({
            level: 'error',
